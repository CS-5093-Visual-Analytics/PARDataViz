...
class RadarDataUI(QMainWindow):
    ...
    def back(self):
        # Move one step back in the scan times, wrapping around if necessary
        self.current_scan_index = (self.current_scan_index - 1) % len(self.scan_times)
        self.update_scan_from_index()

    def forward(self):
        # Move one step forward in the scan times, wrapping around if necessary
        self.current_scan_index = (self.current_scan_index + 1) % len(self.scan_times)
        self.update_scan_from_index()

    def toggle_play_pause(self):
        # Start or stop the timer for looping through scans
        if self.play_timer.isActive():
            self.play_timer.stop()
            self.play_button.setIcon(QIcon.fromTheme("media-playback-start"))  # Play icon
        else:
            self.play_timer.start(1000)  # Adjust interval for playback speed
