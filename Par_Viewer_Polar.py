...
class RadarDataUI(QMainWindow):
    def __init__(self):
        ...
        self.play_timer.timeout.connect(self.forward)  # Connect timer to the forward method
    ...
    def update_ppi_view(self):
        ...
        # Adjust layout to center the plot and leave space on the right for the color bar
    ...
    def toggle_play_pause(self):
        ...
        if self.play_timer.isActive():
            ...
            self.play_button.setIcon(QIcon.fromTheme("media-playback-start"))  # Change icon to "play"
        else:
            self.play_timer.start(1000)  # Start timer with 1-second intervals
            self.play_button.setIcon(QIcon.fromTheme("media-playback-pause"))  # Change icon to "pause"
